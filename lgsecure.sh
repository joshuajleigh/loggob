...
then
	...
	for i in $ENVS; do
		ansible -i $apath$i -m shell -a "cat /var/log/secure|grep -v 'Finished catalog\|freshclam\|deprecat'|cut -d' ' -f5-|sed 's/\[[0-9]\{3,7\}\]\|[0-9]\{0,6\}-\?[a-z]\{2,3\}-[a-z]\{2,8\}[0-9]\{0,2\}//g' |sort|uniq -c|sort -rn|more" $1 -b |sed 's/ \{2,\}/ /g'| sed -e "1d" >> /tmp/$LOG.fetchedseclog
	done
fi
